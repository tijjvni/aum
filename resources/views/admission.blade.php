  ...
  <div role="article" aria-label="An email from Your Al-Ansar University" lang="en"
    ...
    <header>
      ...
      <h1 class="my-4 text-lg font-medium text-green-600">
        Hi Registrar, We just received an Application<br>
      </h1>
    </header>
    ...
    <main>
      <div>
        <h3 class="my-2 text-xl text-purple-500 font-momo">
          The contents of the Applicant are as follows
        </h3>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Name:</b>
          {{ $admission->name . ' ' . $admission->last_name . ' ' . $admission->middle_name ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Email:</b> {{ $admission->email ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Phone:</b> {{ '0' . $admission->phone ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Gender:</b> {{ $admission->gender ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Date of Birth:</b> {{ $admission->dob ?? '' }}
        </p>

        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Nationality:</b> {{ $admission->nationality ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">State of residence:</b> {{ $admission->state ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Address:</b> {{ $admission->address ?? '' }}
        </p>
        <h1 class="">Academic Details</h1>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Application Type:</b> {{ $admission->type ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Jamb Score:</b> {{ $admission->jamb_score ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Jamb Regno:</b> {{ $admission->jamb_regno ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Faculty Applied For:</b>
          {{ 'College of ' . $admission->faculty ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Department and Programme: </b>
          {{ $admission->department ?? '', $admission->course }}
        </p>
        <h1 class="">Parent/Guardian Details</h1>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Name:</b>
          {{ $admission->guardian_name ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Email:</b> {{ $admission->guardian_email ?? '' }}
        </p>
        <p class="p-1 m-0">
          <b class="text-xl font-medium text-black">Phone:</b> {{ '0' . $admission->guardian_phone ?? '' }}
        </p>
      </div>
